...
class PnpHeadPoseEstimator:
    ...
    def __init__(self, dlib_shape_predictor_file_path, cam_w=640, cam_h=480):
        ...
        self._shape_predictor = dlib.shape_predictor(
            dlib_shape_predictor_file_path)
...
if __name__ == '__main__':
    ...
    while (cap.isOpened()):
        ...
        if pitch_yaw_roll is not None:
            ...
            cv2.putText(frame, 'pitch:{:+.2f}'.format(pitch), (0, 20),
                        cv2.FONT_HERSHEY_PLAIN, 1, (0, 0, 255), 1)
            cv2.putText(frame, 'yaw:{:+.2f}'.format(yaw), (0, 35),
                        cv2.FONT_HERSHEY_PLAIN, 1, (0, 0, 255), 1)
            cv2.putText(frame, 'roll:{:+.2f}'.format(roll), (0, 50),
                        cv2.FONT_HERSHEY_PLAIN, 1, (0, 0, 255), 1)
