...
def main(args):
    ...
    with tf.Graph().as_default():
        ...
        image_batch = tf.compat.v1.placeholder(tf.float32, shape=(None, args.image_size, args.image_size, 3),\
                                     name='image_batch')
        landmark_batch = tf.compat.v1.placeholder(tf.float32,
                                                  shape=(None, 196),
                                                  name='landmark_batch')
        attribute_batch = tf.compat.v1.placeholder(tf.int32,
                                                   shape=(None, 6),
                                                   name='attribute_batch')
        euler_angles_gt_batch = tf.compat.v1.placeholder(
        ...
        phase_train_placeholder = tf.compat.v1.placeholder(tf.bool,
                                                           name='phase_train')
...
def train(sess, epoch_size, epoch, list_ops):
    ...
    image_batch, landmarks_batch, attribute_batch, euler_batch = list_ops[
        'train_next_element']
    ...
    for i in range(epoch_size):  #batch_num
        ...
        for k in range(pre_landmarks.shape[0]):
            ...
            for count_point in range(pre_landmarks.shape[1] // 2):  #num points
                error_diff = pre_landmarks[k][(count_point * 2):(
                    count_point * 2 +
                    2)] - landmarks[k][(count_point * 2):(count_point * 2 + 2)]
                error = np.sqrt(np.sum(error_diff * error_diff))
                error_all_points += error
            interocular_distance = np.sqrt(
                np.sum(pow((landmarks[k][120:122] - landmarks[k][144:146]),
                           2)))
