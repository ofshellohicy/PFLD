...
def create_mtcnn(sess, model_path):
    ...
    with tf.variable_scope('pnet'):
        data = tf.compat.v1.placeholder(tf.float32, (None, None, None, 3),
                                        'input')
        pnet = PNet({'data': data})
...
def bulk_detect_face(images, detection_window_size_ratio, pnet, rnet, onet,
        ...
        if numbox > 0:
            ...
            if numbox > 0:
                for k in range(0, numbox):
                    tmp = np.zeros((int(tmph[k]), int(tmpw[k]), 3))
                    tmp[dy[k] - 1:edy[k], dx[k] -
                        1:edx[k], :] = images[index][y[k] - 1:ey[k], x[k] -
                                                     1:ex[k], :]
                    if tmp.shape[0] > 0 and tmp.shape[1] > 0 or tmp.shape[
                            0] == 0 and tmp.shape[1] == 0:
                        tempimg[:, :, :, k] = imresample(tmp, (24, 24))
    ...
    for index, image_obj in enumerate(images_with_boxes):
        ...
        points_per_image[0:5, :] = np.tile(
            w, (5, 1)) * points_per_image[0:5, :] + np.tile(
                image_obj['total_boxes'][:, 0], (5, 1)) - 1
        points_per_image[5:10, :] = np.tile(
            h, (5, 1)) * points_per_image[5:10, :] + np.tile(
