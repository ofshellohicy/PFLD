...
def train_model(loss, global_step, data_num, args):
    ...
    lr_values = [
        args.learning_rate * (lr_factor**x)
        for x in range(0,
                       len(lr_epoch) + 1)
    ]
    lr_op = tf.compat.v1.train.piecewise_constant(global_step, boundaries,
                                                  lr_values)
...
def LandmarkImage(Landmarks, image_size, sigma=None):
    ...
    Pixels = tf.concat([tf.expand_dims(yy, -1),
                        tf.expand_dims(xx, -1)],
                       axis=-1)
    ...
    def Do(L):
        ...
        L = tf.map_fn(DoIn, L)
        L = tf.reshape(tf.reduce_max(L, axis=0),
...
if __name__ == "__main__":
    ...
    landmarks_placeholder = tf.compat.v1.placeholder(tf.float32,
                                                     shape=(None, 18),
                                                     name='landmarks')
